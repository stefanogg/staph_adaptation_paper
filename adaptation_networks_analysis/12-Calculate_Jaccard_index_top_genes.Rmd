...
<!-- ```{r} -->
<!-- x <- unique(df_mutated_genes$patient_episode_unique) -->
<!-- list_pairs <- combn(x = x,  -->
<!--                m = 2, -->
<!--                simplify = F)  -->
<!-- m_pairs <- combn(x = x,  -->
<!--                m = 2, -->
...
df_centrality %>%
  ...
  ggplot(aes(x = sab_duration, y = centrality)) +
  geom_point() +
  theme_bw()

df_centrality %>%
   drop_na(sab_duration) %>%
  ggplot(aes(x = centrality, fill = as.factor(sab_duration >= 7))) +
  geom_density(alpha = .6) +
