---
title: 'Updated operon convergence analysis with CAMERA2 patient-episode analysis: generate comprehensive dataset'
...
Here we create a dataset with convergence data. The whole gene convergence analysis process is described in `Operon_convergence_analysis.Rmd`.
...
```{r}
file.copy("~/Documents/Transfer_with_server/operons", "GSEA/", recursive = T, overwrite = F)

f <- list.files("GSEA/operons", recursive = T,  pattern = "results.gene", full.names = T)
...
f <- list.files("GSEA/operons", recursive = T,  pattern = "results.ontology", full.names = T)
...
df_gsea <- gene_results %>%
  select(operon_id = Locus, nsub, kb, enrich, mutation_compartment) %>%
  left_join(ontology_results %>% select(operon_id = Ontology, glm.pval, mutation_compartment)) %>%
...
```{r}
df_gsea_neb %>%
  saveRDS("processed_data/operon_convergence/df_gsea_neb.Rda")
df_gsea_neb %>%
  write_csv("processed_data/operon_convergence/df_gsea_neb.csv")

df_gsea_neb_with_mutations %>%
  saveRDS("processed_data/operon_convergence/df_gsea_neb_with_mutations.Rda")
df_gsea_neb_with_mutations %>%
  write_csv("processed_data/operon_convergence/df_gsea_neb_with_mutations.csv")
```
